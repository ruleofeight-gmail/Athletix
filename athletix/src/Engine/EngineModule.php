...
class EngineModule implements Module {
	...
	public function register( Plugin $plugin ) {
		...
		$c->bind( 'engine.sport', static function () use ( $plugin ) {
			return new SportEngine( $plugin->config() );
		} );

		$c->bind( 'engine.match', static function () use ( $plugin ) {
			return new MatchEngine( $plugin->events(), $plugin->make( 'repo.match' ), $plugin->logger() );
		} );

		$c->bind( 'engine.standings', static function () use ( $plugin ) {
		...
		$c->bind( 'engine.ranking', static function () use ( $plugin ) {
			return new RankingEngine( $plugin->make( 'engine.standings' ) );
		} );

		$c->bind( 'engine.statistics', static function () use ( $plugin ) {
